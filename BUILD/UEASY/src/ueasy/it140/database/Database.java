...
public class Database extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		queryCampus = "CREATE TABLE " + Table_Campus + "(" + Campus_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " 
				   + Campus_name + " TEXT, " 
				   + Campus_addr + " TEXT, " 
				   + Campus_desc + " TEXT);";
		db.execSQL(queryCampus);

		queryBuilding = "CREATE TABLE " + Table_BuildingLevel + "(" + BuildingLevel_ID	+ " INTEGER PRIMARY KEY AUTOINCREMENT, " 
			  		   + BuildingLevel_BuildingID + " INTEGER, " 
					   + BuildingLevel_BuildingNum + " INTEGER);";
		db.execSQL(queryBuilding);

	queryOtherAmenities = "CREATE TABLE " + Table_Amenities + "(" + Amenities_ID	+ " INTEGER PRIMARY KEY AUTOINCREMENT, " 
							  	   + Campus_ID	+ " INTEGER, " 
								   + Amenities_name + " TEXT, "
								   + Amenities_desc + " TEXT, " 
								   + Amenities_pic + " TEXT, "
								   + Amenities_BldgID + " INTEGER, "
								   + Amenities_BldgLevel + " INTEGER, "
								   + Amenities_latitude + " TEXT," 
								   + Amenities_longitude	+ " TEXT, "
								   + Amenities_catName + " TEXT);";
	db.execSQL(queryOtherAmenities);

	queryRoomUtility = "CREATE TABLE " + Table_RoomUtility + "(" + RoomUtility_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " 
				 			 + RoomUtility_ClassroomID	+ " INTEGER, " 
							 + RoomUtility_Monday + " TEXT, " 
							 + RoomUtility_Tuesday	+ " TEXT, " 
							 + RoomUtility_Wednesday + " TEXT, "
							 + RoomUtility_Thursday + " TEXT, "
							 + RoomUtility_Friday + " TEXT, "
							 + RoomUtility_Saturday + " TEXT, "
							 + RoomUtility_Sunday + " TEXT);";
						  db.execSQL(queryRoomUtility);
						  
	  queryDB = "CREATE TABLE " + Table_DB + "(" + DB_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " 
	 			 + DB_Version	+ " INTEGER);";
			  db.execSQL(queryDB);
...
*/
	...
	public List<DatabaseObject> getAllDatabaseObject(String type) {
		...
		selectQuery = "SELECT  * FROM " + Table_Amenities + " WHERE " + Amenities_catName+"= '"+type+"'";
...
//					+ " LIKE ?", args, null, null, null);
			String rq = "SELECT "+Amenities_name+" FROM " +Table_Amenities+" WHERE "+Amenities_name+" LIKE '"+roomCode+"%'";
//	String rq ="SELECT amenity_name FROM Amenities WHERE amenity_name LIKE '"+roomCode+"%'";
			cursor2 = db.rawQuery(rq, null);
		}else{
		cursor2 = db.query(Table_Amenities, columns2,
				Amenities_catName + "=?", whereArgs, null, null, null);
		}
...
//				name.replaceAll("'", "''");
		selectQuery = "SELECT  * FROM " + Table_Amenities + " WHERE " + Amenities_name +"= "+'"'+name+'"';
...
//						int blevel = cursor.getInt(cursor.getColumnIndex(cNames[i]));
						
//						if(cursor.getString(cursor.getColumnIndex(cNames[i]))!=null)
						data.setBlevels(Integer.parseInt(cursor.getString(cursor
									.getColumnIndex(cNames[i]))));
	...
	public long inserToCampus (int campus_id, String c_name, String c_addr, String c_desc){
		SQLiteDatabase db = this.getWritableDatabase();
		long retVal =0;
		ContentValues values = new ContentValues();
		...
		if(getID(campus_id, Table_Campus, Campus_ID)>0)
		{
			...
			retVal=db.update(Table_Campus, values, Campus_ID+"=?", whereArgs);
		}
		...
		Toast.makeText(this.context, "retVal in DB: "+ retVal, Toast.LENGTH_SHORT).show();
		...
		public long inserToAmenity (String a_catName, int a_id, int campus_id, int bldg_id, int a_bLevel, String a_name, String a_desc, String a_pic, double a_la, double a_longi){
			long retVal =0;
		...
		if(getID(a_id, Table_Amenities, Amenities_ID )>0)
		{
			...
			retVal=database.update(Table_Amenities, values, Amenities_ID+"=?", whereArgs);
		}
		...
		public long inserToBLevel (int bl_id, int bl_bID, int bl_bNum){
			...
			if(getID(bl_id, Table_BuildingLevel, BuildingLevel_ID )>0)
			{
				...
				retVal=db.update(Table_BuildingLevel, values, BuildingLevel_ID+"=?", whereArgs);
			}
		...
		public long inserToRoomUtility (int ru_id, int cr_id, String ru_mon, String ru_tue, String ru_wed, String ru_thu, String ru_fri, String ru_sat, String ru_sun){
			long retVal=0;
			SQLiteDatabase db = this.getWritableDatabase();
			ContentValues values = new ContentValues();
			...
			if(getID(ru_id, Table_RoomUtility, RoomUtility_ID )>0)
			{
				...
				retVal=db.update(Table_RoomUtility, values, RoomUtility_ID+"=?", whereArgs);
			}
	...
	public ArrayList<HashMap<String, String>> getAllOA(String catName) {
		ArrayList<HashMap<String, String>> oaList;
		oaList = new ArrayList<HashMap<String, String>>();
		String selectQuery = "SELECT  * FROM "+Table_OtherAmenities+" where oa_catName="+catName;
	    SQLiteDatabase database = this.getWritableDatabase();
	    Cursor cursor = database.rawQuery(selectQuery, null);
	    if (cursor.moveToFirst()) {
	        do {
	        	...
	        	int ndx1 = cursor.getColumnIndex(OtherAmenities_ID);
				int ndx2 = cursor.getColumnIndex(Campus_ID);
				int ndx3 = cursor.getColumnIndex(Building_ID);
				int ndx4 = cursor.getColumnIndex(OtherAmenities_name);
				int ndx5 = cursor.getColumnIndex(OtherAmenities_desc);
				int ndx6 = cursor.getColumnIndex(OtherAmenities_pic);
				int ndx7 = cursor.getColumnIndex(OtherAmenities_latitude);
				int ndx8 = cursor.getColumnIndex(OtherAmenities_longitude);
				int ndx9 = cursor.getColumnIndex(OtherAmenities_catName);
				int ndx10 = cursor.getColumnIndex(OtherAmenities_bLevel);
	        	
	        	
				map.put(OtherAmenities_ID, cursor.getString(ndx1));
	        	map.put(Campus_ID, cursor.getString(ndx2));
	    		map.put(Building_ID, cursor.getString(ndx3));
	...
	*/
	...
	public int getID (int id, String table, String columnID){
		...
		String[] whereArgs= {""+id};
		Cursor cursor = db.query(table, columns, columnID+"=?", whereArgs, null, null, null);
	...
	public int DBVersion (){
		...
		Cursor cursor = db.query(Table_DB, columns, null, null, null, null, null);
